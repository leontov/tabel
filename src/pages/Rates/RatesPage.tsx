  ...
  return (
    <div className="list-stack">
      <Card title={t('rates.title')}>
        <Field label={t('common.add')} error={errors.title}>
          <input
            value={form.title}
            onChange={(event) => {
        ...
        <Field label={t('rates.base')} error={errors.basePerHour}>
          <input
            ...
            onChange={(event) => {
              setForm((prev) => ({ ...prev, basePerHour: Number(event.target.value) }));
              clearError('basePerHour');
            }}
          />
        </Field>
        <div className="grid-two">
          <Field label={t('rates.overtimeX')} error={errors.overtimeX}>
            <input
              type="number"
              value={form.overtimeX}
              onChange={(event) => {
                setForm((prev) => ({ ...prev, overtimeX: Number(event.target.value) }));
                clearError('overtimeX');
              }}
            />
          </Field>
          <Field label={t('rates.nightX')} error={errors.nightX}>
            <input
              type="number"
              value={form.nightX}
              onChange={(event) => {
                setForm((prev) => ({ ...prev, nightX: Number(event.target.value) }));
                clearError('nightX');
              }}
            />
          </Field>
          <Field label={t('rates.weekendX')} error={errors.weekendX}>
            <input
              type="number"
              value={form.weekendX}
              onChange={(event) => {
                setForm((prev) => ({ ...prev, weekendX: Number(event.target.value) }));
                clearError('weekendX');
              }}
            />
          </Field>
